...
use anyhow::Result;
use syfrah_net::control::{ControlRequest, ControlResponse, send_control_request};
...
pub async fn watch(pin: Option<String>) -> Result<()> {
    ...
    if !store::exists() {
        ...
        syfrah_net::daemon::auto_init(&node_name, 51820, 51821)?;
        println!();
        ...
        tokio::spawn(async move {
            if let Err(e) = syfrah_net::daemon::run_daemon(my_record, &wg_keypair, mesh_secret, pp).await {
