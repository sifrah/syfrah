...
# Verify zone is set
z_before=$(docker exec "e2e-zper-1" syfrah fabric status 2>&1 | grep "Zone:" | awk '{print $2}')
...
z_after=$(docker exec "e2e-zper-1" syfrah fabric status 2>&1 | grep "Zone:" | awk '{print $2}')
