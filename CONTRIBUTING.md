...
## Never commit files from ~/.syfrah/

The `~/.syfrah/` directory contains runtime state, private keys, and secrets generated by the daemon. These files must never be committed to the repository. The `.gitignore` already excludes them, but be especially careful when copying configuration examples or debugging output into commits.

If you accidentally commit any file from `~/.syfrah/`, immediately notify a maintainer so the git history can be cleaned.
